...
const Blacklist = (props) => {
  const handleSubmit = (e) => {
    ...
    props.blacklistTraveller(passenger);
    form.name.value = "";
    form.phone.value = "";
  };
  return (
    <form name="blacklistTraveller" onSubmit={handleSubmit}>
      <input type="text" name="name" placeholder="Name" />
      <input type="text" name="phone" placeholder="Phone Number" />
      <button>Blacklist Traveller</button>
    </form>
  );
};
...
class TicketToRide extends React.Component {
  ...
  async bookTraveller(passenger) {
    ...
    try {
      await graphQLFetch(
        `mutation {
      addTraveller(ticket:{name:"${passenger.name}", phone:${
          passenger.phone
        },bookingTime: "${"2024-03-23"}" }){
        id
      }
    }`
      );
  ...
  async deleteTraveller(passenger) {
    ...
    await graphQLFetch(
      `mutation {
        deleteTraveller(travellername:"${passenger.name}")
      }`
    );
    /*End of Q3*/
  }
  async blacklistTraveller(passenger) {
    ...
    await graphQLFetch(
      `mutation {
  blacklistTraveller(ticket:{name:"${passenger.name}", phone:${
        passenger.phone
      },bookingTime: "${"2024-03-23"}" }){
    id
  }
